...
export default function ApiTestPage() {
  ...
  const testBackendApi = async (): Promise<{ success: boolean; responseTime: number; error?: string; data?: any }> => {
  ...
  const testSimulatorApi = async (): Promise<{ success: boolean; responseTime: number; error?: string; data?: any }> => {
    const startTime = Date.now();
    try {
      // Test endpoint racine
  ...
  const testService = async (serviceIndex: number) => {
    setServices(prev => prev.map((service, index) =>
      index === serviceIndex
        ? { ...service, status: 'loading' as ApiStatus, error: undefined, responseTime: undefined, data: undefined }
        : service
    ));

    let result: { success: boolean; responseTime: number; error?: string; data?: any };
  ...
  const getStatusBadge = (status: ApiStatus) => {
    switch (status) {
      case 'loading':
        return <Badge variant="outline" className="text-yellow-600 border-yellow-300">Test en cours</Badge>;
      case 'success':
        return <Badge variant="outline" className="text-green-600 border-green-300">En ligne</Badge>;
      case 'error':
  ...
  const getEnvironmentBadge = () => {
    switch (ENV.NODE_ENV) {
      ...
      case 'development':
        return <Badge variant="outline" className="text-blue-600 border-blue-300">Développement</Badge>;
      case 'test':
        return <Badge variant="outline" className="text-yellow-600 border-yellow-300">Test</Badge>;
      default:
      ...
      <div className="space-y-6">
          ...
          <CardContent className="pt-6">
            <div className="mb-4 p-3 bg-muted/50 rounded-lg">
              <p className="text-sm text-muted-foreground">
                <strong>Test :</strong> Appel de la route <code>/</code> qui doit retourner <code>{JSON.stringify({hello: "world"})}</code>
              </p>
            </div>
        ...
        <Card>
          ...
          <CardContent className="pt-6">
            <div className="space-y-4">
              <div>
                <p className="text-sm text-muted-foreground mb-2">Variables d'environnement</p>
